...
const catalogNames = {
  new: { name: "New Release Comics", icon: "📚", zone: "new", customer: "Tiny Titan Tim", customerIcon: "🦸" },
  manga: { name: "Manga Volumes", icon: "🌸", zone: "manga", customer: "Mikey", customerIcon: "🎒" },
  cards: { name: "Trading Card Packs", icon: "🃏", zone: "cards", customer: "Card Table Carl", customerIcon: "🧢" },
  back: { name: "Back Issue Bundle", icon: "🗃️", zone: "longboxes", customer: "Sandra", customerIcon: "🧐" },
  figures: { name: "Collectible Figures", icon: "🧸", zone: "rare", customer: "Cosplay Nina", customerIcon: "🧵" },
...
const fallbackBrowsers = [
  ...
  { icon: "🦸", name: "Tim", target: "new", action: "hunts for punching" },
];

const phaseLabels = {
  opening: "Customers enter through the front door.",
  browsing: "They move to real sections based on your inventory, pull lists, and trends.",
...
function customerFromPullRequest(req, index) {
  const meta = catalogNames[req.itemId] || catalogNames.new;
  return {
...
function customerFromInventoryItem(item, index, reason = "sale") {
  const meta = catalogNames[item.id] || catalogNames.new;
  const action = reason === "trend" ? `Trend sale: ${meta.name}` : `Buys ${meta.name}`;
  return {
...
function buildCustomerPlan(save) {
  ...
  pullList.slice(0, 2).forEach(req => {
    plan.push(customerFromPullRequest(req, plan.length));
  });
  ...
  inventoryItems(save).slice(0, 6).forEach(item => {
    ...
    if (plan.some(p => p.target === (catalogNames[item.id]?.zone || "new") && p.type !== "browse")) return;
...
function buildUnifiedFloorMap() {
  ...
  wrapper.innerHTML = `
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
      <div><div style="font-size:11px;font-weight:950;letter-spacing:.14em;text-transform:uppercase;color:#b45309;">Live Floor Plan</div><div style="font-size:13px;font-weight:850;color:#475569;line-height:1.2;">Live Day uses your save: pull lists, stock, trends, then checkout.</div></div>
      <div style="border-radius:999px;background:#0f172a;color:white;padding:7px 11px;font-size:12px;font-weight:950;white-space:nowrap;box-shadow:0 2px 8px rgba(15,23,42,.2);">Smart Map</div>
    </div>
    ...
    <div style="border-radius:18px;background:#fed7aa;border:2px dashed #ea580c;padding:10px;text-align:center;font-size:12px;font-weight:950;color:#7c2d12;">Customer path: Entrance → section picked from your real shop state → Register → day report</div>
  `;
  return wrapper;
}
...
export default function AppV14() {
  ...
  function startLiveDay() {
    ...
    setLiveContext({ trend, day: save.day || 1, stock: inventoryItems(save).reduce((sum, item) => sum + (Number(item.stock) || 0), 0), pulls: Array.isArray(save.pullList) ? save.pullList.length : 0 });
    setCustomers(plan);
    setActivityIndex(0);
    setPhase("opening");
    setLive(true);
  ...
  return <div className="relative min-h-screen">
    <AppV8 />
    <button onClick={startLiveDay} disabled={live} className="fixed left-1/2 top-[3.35rem] z-[65] -translate-x-1/2 rounded-2xl bg-slate-950 px-4 py-2 text-sm font-black text-white shadow-2xl ring-1 ring-white/20 active:scale-95 disabled:bg-slate-500 lg:top-4">
...
function LiveMapOverlay({ customers, phase, activityIndex, context }) {
  ...
  return <motion.div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/45 p-3 backdrop-blur-[2px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    <motion.div initial={{ scale: 0.96, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 20 }} className="relative flex h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] bg-[#fffaf0] p-4 shadow-2xl ring-1 ring-black/10">
      ...
      <div className="mb-3 grid grid-cols-3 gap-2">
        <Mini label="Trend" value={`${context.trend?.icon || "📈"} ${context.trend?.name || "Weekly"}`} />
        <Mini label="Pulls" value={context.pulls} />
        <Mini label="Stock" value={context.stock} />
      </div>
...
function ActivityFeed({ activities, phase }) {
  return <div className="absolute bottom-3 left-3 right-3 z-30 rounded-2xl bg-slate-950/95 p-3 text-white shadow-xl ring-1 ring-white/10">
    ...
    <div className="space-y-1.5">{activities.map((activity, index) => <motion.div key={`${activity}-${index}`} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} className={`rounded-xl px-3 py-2 text-sm font-bold ${index === 0 ? "bg-white text-slate-950" : "bg-white/10 text-slate-100"}`}>{activity}</motion.div>)}</div>
  </div>;
}
