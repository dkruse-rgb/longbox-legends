...
const customerTypes = [
  { icon: "🎒", name: "Mikey", target: "manga", action: "+$13 Manga Volume" },
  { icon: "🧐", name: "Sandra", target: "longboxes", action: "Back issue found" },
  { icon: "📈", name: "Derek", target: "rare", action: "Key issue rumor" },
  { icon: "🧢", name: "Carl", target: "cards", action: "+$11 Card Pack" },
  { icon: "🎨", name: "Ava", target: "stock", action: "Dropped off zines" },
  { icon: "☕", name: "Kim", target: "reading", action: "Browsed and bought" },
  { icon: "🦸", name: "Tim", target: "new", action: "+$8 New Comic" },
];

const phaseLabels = {
  opening: "Customers enter through the front door.",
  browsing: "They move to real shop sections and browse.",
...
function makeCustomers() {
  ...
  return customerTypes.slice(0, count).map((customer, index) => {
  ...
  return `
    <div style="${base}${bg}">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:6px;">
        <div style="font-size:12px;font-weight:950;line-height:1.05;max-width:120px;">${zone.label}</div>
        <div style="font-size:24px;line-height:1;">${zone.icon}</div>
      </div>
      <div style="font-size:10px;font-weight:800;line-height:1.05;opacity:.72;">${zone.desc}</div>
      ${zone.kind === "longbox" ? `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:4px;"><span style="height:13px;background:#334155;border-radius:5px;"></span><span style="height:13px;background:#334155;border-radius:5px;"></span><span style="height:13px;background:#334155;border-radius:5px;"></span></div>` : ""}
    </div>
  `;
}
...
function buildUnifiedFloorMap() {
  ...
  wrapper.innerHTML = `
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
      <div>
        <div style="font-size:11px;font-weight:950;letter-spacing:.14em;text-transform:uppercase;color:#b45309;">Live Floor Plan</div>
        <div style="font-size:13px;font-weight:850;color:#475569;line-height:1.2;">Live Day now uses this same map: entrance → sections → register.</div>
      </div>
      <div style="border-radius:999px;background:#0f172a;color:white;padding:7px 11px;font-size:12px;font-weight:950;white-space:nowrap;box-shadow:0 2px 8px rgba(15,23,42,.2);">Unified Map</div>
    </div>
    ...
    <div style="border-radius:18px;background:#fed7aa;border:2px dashed #ea580c;padding:10px;text-align:center;font-size:12px;font-weight:950;color:#7c2d12;">
      Customer path: Entrance → shop section → Register → day report
    </div>
  `;
  return wrapper;
}
...
function enhanceShopFloor() {
  ...
  if (!section) return;
  if (section.querySelector("[data-longbox-floor-map='true']")) return;
...
export default function AppV13() {
  ...
  useEffect(() => {
    ...
    const activityTimer = setInterval(() => setActivityIndex(i => Math.min(i + 1, customerTypes.length - 1)), 650);
  ...
  return <div className="relative min-h-screen">
    ...
    <AnimatePresence>
      {live && <LiveMapOverlay customers={customers} phase={phase} activityIndex={activityIndex} />}
    </AnimatePresence>
  </div>;
}

function LiveMapOverlay({ customers, phase, activityIndex }) {
  const activities = customers.slice(0, Math.max(1, activityIndex + 1)).map(c => `${c.icon} ${c.name}: ${c.action}`).slice(-4).reverse();
  ...
  >
    <motion.div
      ...
      className="relative flex h-[80vh] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] bg-[#fffaf0] p-4 shadow-2xl ring-1 ring-black/10"
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500 sm:text-base">{phaseLabels[phase] || "The shop is active."}</p>
        </div>
...
function FloorZone({ zone }) {
  ...
  const classes = zone.kind === "register"
    ? "bg-slate-950 text-white"
    : zone.kind === "door"
      ? "bg-gradient-to-br from-emerald-100 to-white text-slate-950"
      : zone.kind === "wall"
        ? "bg-gradient-to-br from-blue-100 to-white text-slate-950"
        : zone.kind === "longbox"
          ? "bg-gradient-to-br from-slate-50 to-white text-slate-950"
          : "bg-white text-slate-950";

  return <div
    className={`absolute overflow-hidden rounded-2xl border border-black/10 p-2 shadow-md ${classes}`}
    style={{ left: `${left}%`, top: `${top}%`, width: `${zone.w}%`, height: `${zone.h}%` }}
  >
    <div className="flex items-start justify-between gap-1">
      <div className="max-w-[7rem] text-[10px] font-black leading-none sm:text-xs">{zone.label}</div>
...
function PathingCustomer({ customer }) {
  ...
  return <motion.div
    ...
    initial={{ left: `${points[0].x}%`, top: `${points[0].y}%`, opacity: 0, scale: 0.8 }}
    animate={{
      left: points.map(p => `${p.x}%`),
      top: points.map(p => `${p.y}%`),
      opacity: [0, 1, 1, 1, 0],
      scale: [0.8, 1, 1.05, 1, 0.85],
    }}
    transition={{ duration: 4.6, delay: customer.delay, times: [0, 0.2, 0.58, 0.82, 1], ease: "easeInOut" }}
  >
    <div className="relative -translate-x-1/2 -translate-y-1/2">
      <motion.div className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-2xl shadow-xl ring-2 ring-amber-300" animate={{ y: [0, -5, 0] }} transition={{ duration: 0.9, repeat: Infinity, delay: customer.delay }}>
        {customer.icon}
      </motion.div>
      ...
      <motion.div
        className="absolute -top-8 left-1/2 min-w-max -translate-x-1/2 rounded-full bg-slate-950 px-2 py-1 text-[10px] font-black text-white shadow-lg"
        initial={{ opacity: 0, y: 6 }}
        animate={{ opacity: [0, 0, 1, 1, 0], y: [6, 6, 0, 0, -6] }}
        transition={{ duration: 4.6, delay: customer.delay, times: [0, 0.35, 0.48, 0.72, 1] }}
      >
...
function ActivityFeed({ activities, phase }) {
  return <div className="absolute bottom-3 left-3 right-3 z-30 rounded-2xl bg-slate-950/95 p-3 text-white shadow-xl ring-1 ring-white/10">
    <div className="mb-2 flex items-center justify-between gap-3">
      <div className="text-xs font-black uppercase tracking-widest text-amber-300">Live Activity</div>
      <div className="text-xs font-black text-slate-300 capitalize">{phase}</div>
    </div>
