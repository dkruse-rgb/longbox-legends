...
const STORAGE_KEY = "longbox-legends-save-v2";
...
function loadGame() {
  try {
    ...
    if (v2) return { ...start, ...v2 };
    const v1 = JSON.parse(localStorage.getItem("longbox-legends-save-v1"));
    if (v1) return { ...start, ...v1 };
  } catch {
    return start;
  }
  return start;
...
export default function App() {
  ...
  function pushLog(line) {
    setGame(g => ({ ...g, log: [line, ...g.log].slice(0, 10) }));
  }
  ...
  function buyItem(item) {
    ...
    setGame(g => ({ ...g, cash: g.cash - item.cost, inventory: addStock(g.inventory, item), log: [`Bought ${item.units}x ${item.name} for ${cash(item.cost)}.`, ...g.log].slice(0, 10) }));
  ...
  function hire(person) {
    ...
    setGame(g => ({ ...g, cash: g.cash - person.cost, rep: clamp(g.rep + person.rep, 0, 100), staff: [...g.staff, person.id], log: [`Hired ${person.name} for ${cash(person.cost)}/day.`, ...g.log].slice(0, 10) }));
  ...
  function sellRare(uid) {
    ...
    if (!item) return;
    setGame(g => ({ ...g, cash: g.cash + item.value, xp: g.xp + Math.floor(item.value / 5), rare: g.rare.filter(x => x.uid !== uid), lifetimeSales: g.lifetimeSales + item.value, log: [`Sold ${item.name} for ${cash(item.value)}. Painful goodbye, beautiful margin.`, ...g.log].slice(0, 10) }));
    notify(`Sold ${item.name} for ${cash(item.value)}`);
  }
  ...
  function host(ev) {
    ...
    setGame(g => ({ ...g, cash: g.cash - ev.cost + sales, rep: clamp(g.rep + ev.rep + bonus, 0, 100), hype: clamp(g.hype + ev.visitors, 0, 40), xp: g.xp + ev.visitors * 8, lifetimeVisitors: g.lifetimeVisitors + ev.visitors, lifetimeSales: g.lifetimeSales + sales, log: [`${ev.name} brought ${ev.visitors} extra visitors and earned ${cash(sales)} in buzz sales.`, ...g.log].slice(0, 10) }));
  ...
  function nextDay() {
    ...
    for (let i = 0; i < visitors; i++) {
      ...
      if (choices.length && Math.random() < chance) {
        ...
        if (lines.length < 4) lines.push(`${c.icon} ${c.name} bought ${item.name} for ${cash(price)}.`);
      } else if (!choices.length) {
        repDelta--;
        if (lines.length < 4) lines.push(`${c.icon} ${c.name} found empty shelves. The aura took damage.`);
      }
    }
    ...
    const headline = sales === 0 ? "Rough day at the longboxes" : net >= 0 ? "Solid day behind the counter" : "Good traffic, thin wallet";
    ...
    setGame(g => ({
      ...
      log: [`Day ${g.day}: ${visitors} visitors, ${sales} sales, ${cash(gross)} gross. Rent/payroll: ${cash(payroll + rent)}. Net: ${cash(net)}.`, ...lines, ...g.log].slice(0, 10)
    }));
  ...
  function reset() {
    ...
    localStorage.removeItem("longbox-legends-save-v1");
    setGame(start);
    setTab("shop");
    setReport(null);
  ...
  return <div className="min-h-screen bg-[#f6efe3] text-slate-950 pb-24 lg:pb-8">
    <div className="mx-auto max-w-7xl lg:p-6">
      <div className="overflow-hidden bg-[#fffaf0] shadow-2xl ring-1 ring-black/5 lg:rounded-[2rem]">
          ...
          <div className="relative overflow-hidden px-4 py-4 sm:px-6 lg:px-8 lg:py-7">
            ...
            <div className="relative flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="hidden max-w-2xl text-sm text-slate-300 sm:mt-2 sm:block">A cozy comic shop tycoon about comics, cards, manga, rare finds, and regulars who slowly become your people.</p>
              </div>
        ...
        <section className="grid grid-cols-3 gap-2 p-3 sm:grid-cols-6 lg:p-4">
          <Stat label="Cash" value={cash(game.cash)} hot />
          <Stat label="Rep" value={`${game.rep}/100`} />
          <Stat label="Stock" value={stock} />
          <Stat label="Hype" value={game.hype} />
        ...
        <section className="px-3 pb-3 lg:px-4">
          <div className="rounded-[1.75rem] bg-slate-950 p-4 text-white shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-amber-300">Next Goal</div>
                <div className="mt-1 text-lg font-black">{goal.label}</div>
              </div>
            ...
            <div className="mt-3 h-3 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-amber-400 transition-all" style={{ width: `${clamp((goal.progress / goal.target) * 100, 0, 100)}%` }} />
            </div>
          </div>
        </section>

        <nav className="hidden border-y border-black/5 bg-white/70 px-4 py-3 lg:block">
          <div className="flex gap-2 overflow-x-auto pb-1">{tabs.map(([id, label, icon]) => <TabButton key={id} active={tab === id} onClick={() => setTab(id)} icon={icon} label={label} />)}</div>
        </nav>

        <main className="grid gap-4 p-3 lg:grid-cols-[1.35fr_.85fr] lg:p-4">
          <section className="min-h-[520px]">
            <AnimatePresence mode="wait">
              <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .18 }}>
                {tab === "shop" && <ShopView game={game} traffic={traffic} stock={stock} nextDay={nextDay} />}
                {tab === "buy" && <Grid title="Distributor Market" sub="Pick what your customers will care about. Empty shelves are where reputation goes to die." items={catalog} action={buyItem} cash={game.cash} />}
                ...
                {tab === "staff" && <Grid title="Hire Staff" sub={`Payroll right now: ${cash(payroll)}/day. Good staff hurts the wallet and heals the shop.`} items={staff} action={hire} cash={game.cash} owned={game.staff} daily />}
          ...
          <aside className="space-y-4">
            <Panel title="City Around You" sub="Your little shop has to pull the whole neighborhood into orbit.">
              <div className="grid grid-cols-4 gap-2 rounded-[1.5rem] bg-slate-100 p-2">{["🏪", "📚", "☕", "🛣️", "🏠", "🎮", "🍕", "📦", "🎭", "🏫", "🌳", "🚌"].map((x, i) => <div key={i} className={`flex aspect-square items-center justify-center rounded-2xl text-2xl shadow-sm ring-1 ring-black/5 ${i === 1 ? "bg-amber-200" : "bg-white"}`}>{x}</div>)}</div>
            </Panel>
            <Panel title="Regulars Book" sub="Customers become part of the story.">
              <div className="flex flex-wrap gap-2">{game.regulars.length === 0 ? <span className="text-sm text-slate-500">Open the shop to meet your first regulars.</span> : game.regulars.map(n => { const c = customers.find(x => x.name === n); return <Pill key={n}>{c?.icon} {n}</Pill>; })}</div>
            </Panel>
            <section className="rounded-[1.75rem] bg-slate-950 p-5 text-white shadow-sm">
              <h2 className="text-xl font-black">Shop Feed</h2>
              <p className="mb-3 text-sm text-slate-400">The latest counter gossip.</p>
              <div className="space-y-3">{game.log.map((line, i) => <div key={`${line}-${i}`} className="rounded-2xl bg-white/10 p-3 text-sm text-slate-100 ring-1 ring-white/10">{line}</div>)}</div>
            </section>
            <Panel title="Lifetime">
              <div className="grid grid-cols-2 gap-3"><Mini label="Sales" value={cash(game.lifetimeSales)} /><Mini label="Visitors" value={game.lifetimeVisitors} /></div>
            </Panel>
          </aside>
        </main>
      </div>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-white/95 px-2 pb-[calc(env(safe-area-inset-bottom)+.45rem)] pt-2 shadow-2xl backdrop-blur lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-6 gap-1">{tabs.map(([id, label, icon]) => <button key={id} onClick={() => setTab(id)} className={`rounded-2xl px-1 py-2 text-center transition active:scale-95 ${tab === id ? "bg-slate-950 text-white" : "text-slate-500"}`}><div className="text-lg leading-none">{icon}</div><div className="mt-1 text-[10px] font-black">{label}</div></button>)}</div>
    </nav>

    <AnimatePresence>
      {toast && <motion.div initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} className="fixed left-3 right-3 top-3 z-50 mx-auto max-w-md rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-2xl ring-1 ring-white/10">{toast}</motion.div>}
    </AnimatePresence>
  ...
  return <div className="grid gap-4 xl:grid-cols-[1fr_.85fr]">
    <Panel title="Shop Floor" sub={`Expected traffic: ${traffic} customers/day`}>
      <div className="mb-4 grid grid-cols-3 gap-2">
        <Mini label="Shelves" value={stock} />
        <Mini label="Upgrades" value={game.upgrades.length} />
        <Mini label="Staff" value={game.staff.length} />
      </div>
    ...
    <Panel title="Inventory" sub="Keep shelves full or customers get tragic.">
      {game.inventory.length === 0 && <Notice>No regular inventory yet. Hit Buy and stock the shelves before opening too many days.</Notice>}
      <div className="space-y-3">{game.inventory.map(x => <Row key={x.id} icon={x.icon} title={x.name} detail={`Sells around ${cash(x.price)}`} right={`${x.stock} left`} />)}</div>
    </Panel>
  </div>;
}
  ...
  return <AnimatePresence>{report && <motion.div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-3 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
    <motion.div initial={{ y: 60, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 60, scale: .98 }} onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-[2rem] bg-white p-5 shadow-2xl">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div><div className="text-xs font-black uppercase tracking-widest text-amber-600">Day {report.day} Report</div><h2 className="text-2xl font-black text-slate-950">{report.headline}</h2></div>
        <div className="rounded-2xl bg-slate-100 p-3 text-3xl">{report.featured.icon}</div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Mini label="Visitors" value={report.visitors} /><Mini label="Sales" value={report.sales} /><Mini label="Gross" value={cash(report.gross)} /><Mini label="Net" value={cash(report.net)} /></div>
      <div className="mt-4 space-y-2">{report.lines.length ? report.lines.map((line, i) => <div key={i} className="rounded-2xl bg-slate-950 p-3 text-sm font-semibold text-white">{line}</div>) : <Notice>Nobody bought anything today. The longboxes were quiet. Too quiet.</Notice>}</div>
...
function Card({ item, disabled, onClick, button = "Buy", note, rare }) {
  ...
  return <div className={`rounded-[1.75rem] bg-gradient-to-br ${gradient} p-4 ring-1`}><div className="mb-2 flex items-start justify-between"><div className="text-4xl">{item.icon}</div>{item.grade && <Pill>{item.grade}</Pill>}</div><h3 className="text-lg font-black">{item.name}</h3>{item.role && <div className="text-sm font-bold text-slate-500">{item.role}</div>}<p className="mt-1 min-h-12 text-sm text-slate-600">{item.desc}</p><div className="my-4 flex flex-wrap gap-2"><Pill>{item.cost ? cash(item.cost) : ""}{item.cost && item.role ? "/day" : ""}</Pill>{item.units && <Pill>{item.units} units</Pill>}{item.price && <Pill>Base {cash(item.price)}</Pill>}{item.value && <Pill>Value {cash(item.value)}</Pill>}{item.rep !== undefined && <Pill>Rep +{item.rep}</Pill>}{item.traffic !== undefined && <Pill>Traffic +{item.traffic}</Pill>}{item.visitors && <Pill>{item.visitors} visitors</Pill>}</div>{note && <p className="mb-3 text-xs font-black text-rose-600">{note}</p>}<Button gold={rare || button.includes("Host") || button.includes("Acquire")} disabled={disabled} onClick={onClick}>{button}</Button></div>;
}

function Grid({ title, sub, items, action, cash: moneyNow, owned = [], daily }) {
  return <Panel title={title} sub={sub}><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map(item => <Card key={item.id} item={item} disabled={moneyNow < item.cost || owned.includes(item.id)} onClick={() => action(item)} button={owned.includes(item.id) ? (daily ? "Hired" : "Owned") : (daily ? "Hire" : "Buy")} />)}</div></Panel>;
}
